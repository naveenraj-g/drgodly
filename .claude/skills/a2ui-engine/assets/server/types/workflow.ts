 ...
 * Layer: types
 *
 * Shared interfaces for the A2UI workflow system. Used by the API routes
 * (workflow, step, submit) and the client-side A2UI components.
 */
...
export interface WorkflowDefinition {
   ...
   * workflows (read-only dashboards) are listed on analysis-only pages;
   * everything else (including workflows that omit this field entirely) is
   * treated as "chat" and listed on general-purpose chat pages. Purely a
   * launcher-filtering concern — has no effect on how the workflow itself
   * executes.
   */
...
export interface ContextResolverDef {
  ...
  method?: "GET" | "POST";
  /**
   * Key into GRAPHQL_DOCUMENTS (a2ui/schemas/graphql) — required when
   * type is "graphql".
   */
