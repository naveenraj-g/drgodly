 ...
 *   - "http"    — calls your backend directly using the URL declared in the
 *                 action definition.
 *   - "graphql" — calls A2UI_GRAPHQL_URL via runGraphQLResolverOrAction(),
 *                 looking up the query/mutation document by the action's
 *                 graphql_document key (see client/a2ui/schemas/graphql —
 *                 ships empty; add your own named documents there).
 *
 * Authorization: with A2UI_AUTH_MODE=none (the default), every request is
 * allowed. With A2UI_AUTH_MODE=jwt, re-validates that the caller holds all
 * required_permissions for the workflow (403 if missing) — the workflow
 * JSON is re-sent by the client on every submit, so forged requests are
 * caught here before any backend call.
 *
 * Flow:
 ...
 *   5. Obtain a bearer token (undefined when auth is disabled).
 *   6. Unpack formData — A2UI forms dispatch { formData: "<JSON string>" } so
 *      we JSON.parse if the formData key holds a string.
 *   7. Strip empty/null fields; for "http" actions interpolate the action URL
 *      with sessionContext.
 *   8. If the action declares a validation_schema, validate + transform with
 *      Zod (see client/a2ui/schemas/validation — ships empty; add your own
 *      named schemas there).
 *   9. Call the backend (REST fetch or GraphQL request) with a bearer token
 *      (when configured) and the cleaned payload. For iterate_key steps,
 *      submit each array item individually.
 *   10. Map the response through the step's context.outputs to extract named values.
 *   11. Return success + merged sessionContext + nextStepIndex.
 *
 * Request body:
 *   {
...
import { VALIDATION_SCHEMAS } from "@/modules/client/a2ui/schemas/validation";
...
export async function POST(req: Request) {
  ...
  // Re-pin user_id/org_id/backend_url to the current identity before this
  // context reaches any backend call. sessionContext is re-sent by the
  // client on every submit, so without this a forged org_id/user_id could
  // ride straight into a Zod-validated create/update payload below.
  const sessionContext = buildBaseContext(rawSessionContext, identity);

  // Always prefer the registry's current workflow definition over the
  // client's snapshot — the client may be replaying a workflow JSON it
  // fetched before a server-side edit (new steps, changed actions), same
  ...
  // Re-validate permissions before executing the action. A forged request
  ...
  try {
    ...
    // GraphQL actions have no URL to interpolate — the single
    // A2UI_GRAPHQL_URL endpoint plus the resolved document decide where the
    // call goes.
