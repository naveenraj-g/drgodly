/**
 * useA2uiSend — message-sending and workflow-triggering hooks for A2UIChatContainer.
 *
...
/** Parameters accepted by useA2uiSend. */
export interface UseA2uiSendParams {
  userId: string;
  orgId?: string | null;
...
/** Shape returned by useA2uiSend. */
export interface A2uiSendHandlers {
  /**
 ...
 * @returns Send callbacks consumed by A2UIChatContainer.
 */
export function useA2uiSend({
  userId,
  orgId,
  ensureSession,
  persistMessage,
...
}: UseA2uiSendParams): A2uiSendHandlers {
  const {
    input,
    loading,
    addMessage,
  ...
  const { updateSessionTitle } = useA2uiChatStore();
