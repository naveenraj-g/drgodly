/**
 * useA2uiWorkflow — workflow step loading, skip, and abandon hooks.
 *
...
import { useA2uiChatStore, a2uiChatStore } from "../../stores/a2ui-chat.store";
...
/** Parameters accepted by useA2uiWorkflow. */
export interface UseA2uiWorkflowParams {
  persistMessage: PersistMessageFn;
  dbWorkflowStateId: string | null;
  setDbWorkflowStateId: (id: string | null) => void;
}

/** Shape returned by useA2uiWorkflow. */
export interface A2uiWorkflowHandlers {
  /**
      ...
      try {
        ...
        if (data.type === "workflow_complete") {
          if (workflow.completion?.message) {
            ...
            if (persist) {
              const sid = a2uiChatStore.getState().activeSessionId;
              if (sid)
