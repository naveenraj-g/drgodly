/**
 * useA2uiSession — session management hook for A2UIChatContainer.
 *
 * Layer: client / a2ui / components / chat
 *
 * Encapsulates all session lifecycle callbacks:
 *   - Loads the session list on mount.
 *   - ensureSession: creates a new session on first send, silently updating
 *     the browser URL without causing a navigation or remounting the component.
 *   - persistMessage: fire-and-forget message write.
 *   - startNewChat: clears store state or navigates to the blank landing URL.
 *   - openSession: navigates to an existing session URL.
 *   - handleDeleteSession: removes a session from the sidebar + store.
 */

import { useCallback, useEffect } from "react";
import type { AppRouterInstance } from "next/dist/shared/lib/app-router-context.shared-runtime";
import { useChatStore } from "@/modules/client/a2ui/store/chat-store";
...
import {
  ...
  pinSessionAction,
  addMessageAction,
} from "@/lib/a2ui/session-actions";
import { titleFromMessage } from "./utils";

/** Parameters accepted by useA2uiSession. */
export interface UseA2uiSessionParams {
  userId: string;
  orgId?: string | null;
  /** Session UUID from the URL params — undefined on the blank new-chat page. */
  urlSessionId?: string;
  locale: string;
  /** Base path without locale prefix, e.g. "/chat". */
  basePath: string;
...
/** Shape returned by useA2uiSession. */
export interface A2uiSessionHandlers {
  /**
   * Returns the active session ID. Creates a new session on first call,
  ...
  /**
   * Persists a single message. Errors are swallowed — the message is
   * already in the Zustand store so the UI is unaffected.
   */
  persistMessage: (
    sessionId: string,
    params: {
  ...
  /** Deletes a session from the sidebar and store; navigates away if it was active. */
...
/**
 * Hook providing session management callbacks for A2UIChatContainer.
 *
 * @param params - Session configuration and dependencies.
 * @returns Session management callbacks.
 */
export function useA2uiSession({
  userId,
  orgId,
  urlSessionId,
  locale,
...
}: UseA2uiSessionParams): A2uiSessionHandlers {
  const {
    activeSessionId,
    setSessions,
    setActiveSessionId,
  ...
  // Load the session list whenever the userId changes (mount + org switch).
  useEffect(() => {
    let cancelled = false;
  ...
  /**
   * Writes a message row. Fire-and-forget: the message is already visible
   * in the Zustand store so a write failure is non-fatal.
   */
  ...
  /** Removes a session from the list + store and navigates away if it was active. */
  ...
  /** Updates the session title in the store and backing storage. */
  const handleRenameSession = useCallback(
    async (sessionId: string, title: string) => {
      updateSessionTitle(sessionId, title);
      await renameSessionAction({ id: sessionId, title });
    },
    [updateSessionTitle],
  );

  /** Toggles the pinned flag in the store and backing storage. */
  const handlePinSession = useCallback(
    async (sessionId: string, pinned: boolean) => {
      setPinned(sessionId, pinned);
      await pinSessionAction({ id: sessionId, pinned });
    },
    [setPinned],
  );
