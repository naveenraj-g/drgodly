/**
 * useA2uiDispatch — A2UI form-submit dispatch listener for A2UIChatContainer.
 *
...
import { a2uiChatStore } from "../../stores/a2ui-chat.store";
import type { IMessageProcessor } from "@/modules/client/a2ui/rendering/processor";
import {
...
/** Parameters accepted by useA2uiDispatch. */
export interface UseA2uiDispatchParams {
    ...
    const handleDispatch = async (event: Event) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          const toolSid = a2uiChatStore.getState().activeSessionId;
          if (toolSid) {
          ...
          if (data.nextStepIndex !== null) {
            ...
            if (currentWorkflow.completion?.message) {
              ...
              const completeSid = a2uiChatStore.getState().activeSessionId;
              if (completeSid)
          ...
          const errSid = a2uiChatStore.getState().activeSessionId;
          if (errSid) {
        ...
        const netSid = a2uiChatStore.getState().activeSessionId;
        if (netSid) {
