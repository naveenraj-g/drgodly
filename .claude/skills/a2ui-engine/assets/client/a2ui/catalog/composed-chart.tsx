...
import type { ComposedChartNode } from "../types";
...
interface Props {
  ...
  surfaceId: string;
  component: ComposedChartNode;
  weight?: string | number;
}

/**
 * Composed chart — bar series and line series plotted against the same
 * category axis. Built for correlating two different-scale metrics over the
 * same timeline (e.g. daily step count as bars against resting heart rate
 * as an overlaid line) — something no single typed chart above covers.
 */
export function ComposedChart({ processor, surfaceId, component, weight = "initial" }: Props) {
  ...
  const barSeries = resolvePrimitive(component.properties.barSeries) ?? [];
  const lineSeries = resolvePrimitive(component.properties.lineSeries) ?? [];
          ...
          <ChartContainer config={chartConfig} className="h-full w-full !aspect-auto">
            <RechartsComposed data={data}>
              ...
              <XAxis
                ...
                tick={{ fontSize: 11 }}
              />
              <YAxis yAxisId="left" tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 11 }} />
              <YAxis
                yAxisId="right"
                orientation="right"
                tickLine={false}
