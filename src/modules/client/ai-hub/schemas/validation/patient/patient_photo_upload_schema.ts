/**
 * patient_photo_upload_schema — Zod transform for a2ui patient photo upload.
 *
 ...
 * transforms it into the payload expected by POST /patients/{id}/photos:
 *   { url, content_type, size, title, creation }
 *
 * Used by the `add_patient_photo` action in the `create_patient` workflow via
 * the VALIDATION_SCHEMAS registry in /api/workflow/submit.
 */
 ...
 *
 * Output (to POST /patients/{id}/photos):
 *   { url, content_type, size, title, creation }
 */
export const patientPhotoUploadSchema = z
  .object({
    /** Structured file record written by the a2ui FileUpload component after upload. */
    profile_photo: z.object({
      /** FileNest file ID — stored as patient.photo[].url in FHIR. */
      fileId: z.string().min(1, "No file uploaded"),
      /** Original filename — stored as patient.photo[].title. */
      filename: z.string(),
      /** MIME type — stored as patient.photo[].content_type. */
      contentType: z.string(),
      /** File size in bytes — stored as patient.photo[].size. */
      sizeBytes: z.number(),
    }),
  })
