...
interface DataTableExportButtonProps<TData>
  ...
  table: Table<TData>;
  /**
   * Base filename shown in the dialog filename input (no extension).
   * @default "export"
   */
  filename?: string;
  /**
   * Dataset title shown in the dialog header and embedded in PDF exports.
   * @default "Data"
   */
  title?: string;
  /**
   * Optional async handler for server-side full-export.
   * When provided, scope "All filtered rows" delegates here instead of using
   * in-memory rows. Receives the selected format and column list.
   *
   * @param format - Export format chosen by the user.
 ...
 *
 * Drop this into any toolbar alongside DataTableViewOptions. The dialog
 * handles all format/scope/column selection — no configuration needed here.
 *
 * @param table - TanStack Table instance.
