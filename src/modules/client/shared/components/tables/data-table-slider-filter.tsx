...
/** Two-element tuple representing [min, max] */
type RangeValue = [number, number];

/** Validates that a value is a [number, number] tuple */
...
/** Coerces an unknown filter value to a RangeValue or undefined */
...
interface DataTableSliderFilterProps<TData> {
  /** The TanStack column to set range filter values on */
...
/**
 * Range slider filter that renders a popover with:
 * - From / To numeric inputs for precise entry
 * - A dual-thumb Slider for visual range selection
...
export function DataTableSliderFilter<TData>({
  ...
  // Compute slider bounds from column meta or faceted values
  const { min, max, step } = React.useMemo<{
    ...
    if (defaultRange && isValidRange(defaultRange)) {
      ...
      // Use faceted min/max when available (requires getFacetedMinMaxValues row model)
    ...
    // Auto-derive step size based on range magnitude
  ...
  // Current range value (falls back to [min, max] when no filter active)
  const range = React.useMemo<RangeValue>(
    () => columnFilterValue ?? [min, max],
    [columnFilterValue, min, max],
  );

  /** Formats a number for display in the trigger button label */
  const fmt = (v: number) =>
