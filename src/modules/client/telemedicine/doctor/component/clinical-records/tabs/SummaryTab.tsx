/**
 * SummaryTab — appointment context, encounters, and the SOAP note editor.
 *
 * Layer: client / telemedicine / doctor / component / clinical-records / tabs
 *
 * The landing tab of the clinical workspace. Shows what the visit was, which
 * Encounter(s) it produced, and the editable SOAP note that everything else in
 * the workspace is derived from.
 *
...
import { CalendarDays, Clock, FileText, Layers, Stethoscope } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { SoapEditor } from "../../appointment-review/soap/SoapEditor";
...
interface SummaryTabProps {
  ...
  /** Current SOAP note state (controlled by ClinicalWorkspace). */
  soap: SoapNote;
  /** Called with the full updated note whenever any SOAP field changes. */
...
/**
 * Renders visit context plus the SOAP editor.
 *
 * @param appointment - Appointment being documented.
 * @param encounters - Encounters produced by the appointment.
 * @param soap - Current SOAP note.
 * @param onSoapChange - SOAP change handler.
 */
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold">SOAP Note</p>
          <span className="text-xs text-muted-foreground">
            Saved as a draft — publish to write it to the record
          </span>
        </div>
        <SoapEditor soap={soap} onChange={onSoapChange} />
      </div>
    </div>
  );
}
