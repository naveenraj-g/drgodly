/**
 * IdentifiersTab — FHIR business identifiers for the practitioner (NPI,
 * license, DEA, etc.).
 *
 * `use` and `type` are both sourced live from the FHIR terminology server
 * via TerminologySelect — confirmed via the A2UI reference form
 * practitioner_identifier_form.json (resource="Patient" field="identifier.use"
 * and field="identifier.type", the latter a CodeableConcept).
 */
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input {...field} value={field.value ?? ""} placeholder="NPI-1234567890" />
                  )}
                />
              </Field>
            </div>
                  ...
                  <TerminologySelect
                    ...
                    value={
                      field.value
                        ? ({
                            code: field.value,
                            system: form.getValues(`identifier.${i}.type_system`) ?? "",
                            display: form.getValues(`identifier.${i}.type_display`) ?? "",
                          } as TCodeableConcept)
                        : null
                    }
                    onChange={(value) => {
                      if (!value || typeof value !== "object") return;
                      form.setValue(`identifier.${i}.type_code`, value.code);
                      form.setValue(`identifier.${i}.type_system`, value.system);
                      form.setValue(`identifier.${i}.type_display`, value.display);
                    }}
                    placeholder="Search identifier type…"
                  />
                )}
              />
            </Field>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input {...field} value={field.value ?? ""} placeholder="http://hl7.org/fhir/sid/us-npi" />
                  )}
                />
              </Field>
              <Field>
