 ...
 *
 * UI mirrors TextConsultation: Brain-icon header with live status dot,
...
export function VoiceConsultationTest({
  ...
  const startCall = useCallback(async () => {
    ...
    try {
      ...
      const tokenRes = await fetch("/api/consultation-voice-agent");
      if (!tokenRes.ok) {
      ...
      // 2. Open WebSocket
      console.log("[VoiceConsultationTest] connecting to WS:", wsUrl);
      ...
      ws.onmessage = (event: MessageEvent) => {
        try {
          ...
          if (data.type === "text" && data.text) {
            ...
            console.log("[VoiceConsultationTest] text (agent):", data.text.slice(0, 100));
            flushPendingUserTranscript();
