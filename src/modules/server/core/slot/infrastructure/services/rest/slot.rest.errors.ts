 ...
 * domain error. Imported by SlotCoreRestService.
 *
 ...
 * @throws ConflictError      on HTTP 409 (slot unavailable in booking context)
 * @throws RateLimitError     on HTTP 429
 * @throws BadGatewayError    on all other non-2xx responses
 */
export function handleSlotApiError(error: AxiosError): never {
  ...
  switch (error.response?.status) {
    ...
    case 409:
      // Slot unavailable — another booking claimed it first
      throw new ConflictError(message);
    case 429:
      throw new RateLimitError(message);
    default:
      throw new BadGatewayError(
        `Slot API error ${error.response?.status ?? "unknown"}: ${message}`,
      );
  }
}
