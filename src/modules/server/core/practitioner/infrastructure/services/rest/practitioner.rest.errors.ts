/**
 * Practitioner REST API error mapper.
 *
 * Layer: server / core / practitioner / infrastructure / services / rest
 *
 * Standalone function (no class dependency) that maps an AxiosError to the
 * appropriate domain error. All REST sub-services import this single function.
 *
  ...
  switch (error.response?.status) {
    ...
    default:
      throw new BadGatewayError(
        `Practitioner API error ${error.response?.status ?? "unknown"}: ${message}`,
      );
  }
}
