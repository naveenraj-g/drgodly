/**
 * handlePatientApiError — Axios → domain error mapper for Patient REST services.
 *
 * Layer: infrastructure / services / rest (shared utility)
 *
 * Thin wrapper around the shared handleFhirApiError. All 10+ Patient sub-service
 * classes import this function so they need no direct dependency on the shared module.
 */
...
/**
 * Maps an AxiosError from the fhir-gql Patient API to a typed domain error and throws it.
...
export function handlePatientApiError(error: AxiosError): never {
  handleFhirApiError(error, "Patient");
}
