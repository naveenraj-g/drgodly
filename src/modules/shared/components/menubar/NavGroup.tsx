    ...
    <SidebarMenuItem>
      <SidebarMenuButton asChild isActive={checkIsActive(href, item)} tooltip={item.title}>
        <Link href={item.url} onClick={() => setOpenMobile(false)}>
          {item.icon && <DynamicLucideIcon name={item.icon} className={item.isLoading ? "animate-spin" : undefined} />}
              ...
              <SidebarMenuSubItem key={subItem.title}>
                <SidebarMenuSubButton asChild isActive={checkIsActive(href, subItem)}>
                  <Link href={subItem.url} onClick={() => setOpenMobile(false)}>
                    {subItem.icon && <DynamicLucideIcon name={subItem.icon} />}
          ...
          {item.items.map((sub) => (
            <DropdownMenuItem key={`${sub.title}-${sub.url}`} asChild>
              <Link href={sub.url} className={checkIsActive(href, sub) ? "bg-secondary" : ""}>
                {sub.icon && <DynamicLucideIcon name={sub.icon} />}
